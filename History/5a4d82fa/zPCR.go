...
var nseIndiceInstrumentsSubscribed = map[string]struct{}{
	"Nifty 50":          {},
	"Nifty Consumption": {},
	"Nifty IT":          {},
	"Nifty Pharma":      {},
...
func isNSEIndiceInstrumentsSubscribed(instrument smartapi.InstrumentRecord) bool {
	_, present := nseIndiceInstrumentsSubscribed[instrument.Symbol]
	return instrument.ExchSeg == "NSE" && present
}
	...
	for _, interval := range data.HistoricTableIntervals {
		...
		for _, exchgCode := range smartapi.CodeExchangeTypes {
			...
			go func(interval data.Interval, exchangeCode string) {
				...
				logger.Log.Info().
					Str("interval", intervalName).
					Str("exchangeCode", exchangeCode).
					Msg("Reading historical data for:")
				...
				logger.Log.Info().
					Str("interval", intervalName).
					Str("exchangeCode", exchangeCode).
					Int("historical data count", len(historicalCandleData)).
					Msg("historical data count for:")
...
func InitializeAlphaHftSOD(smartAPITotp string, createTables bool, populateOldHistoricData bool,
	...
	logger.Log.Info().
		...
		Float64("maxStrike", maxStrike).
		Float64("minStrike", minStrike).
		Msg("Nifty Indice Last Value And Max and Min Strike")
	...
	shouldSkip := func(instrument smartapi.InstrumentRecord) bool {
		...
		if instrumentFilterFlag == "SUBSCRIPTION_NSEINDICES" {
			return !isNSEIndiceInstrumentsSubscribed(instrument)
		}

		if instrumentFilterFlag == "SUBSCRIPTION_NIFTY50" {
			return !isNifty50(instrument) && !isSubscribed(instrument) && !isNSEIndiceInstrumentsSubscribed(instrument)
		}
		...
		if instrumentFilterFlag == "NIFTYOPTNEAR" &&
			...
			if instrumentStrike > minStrike && instrumentStrike < maxStrike {
				return false
			}
		...
		if instrumentFilterFlag == "INDEXOPTIONS" &&
			instrument.ExchSeg == "NFO" &&
			instrument.InstrumentType == "OPTIDX" &&
		...
		if instrumentFilterFlag == "INDEXFUTURES" &&
			instrument.ExchSeg == "NFO" &&
		...
		if instrumentFilterFlag == "CommoditiesAndFutures" {
			if instrument.ExchSeg == "MCX" &&
				(instrument.InstrumentType == "COMDTY" || instrument.InstrumentType == "FUTCOM") {
				return false
			}
		}
