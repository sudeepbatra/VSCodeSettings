...
func init() {
	...
	initialzeBotCmd.Flags().StringVarP(&intervalFilterFlag, "intervalFilterFlag", "t", "ALL", "Specify the interval to fetch the historical data for")

	startBotCmd.Flags().BoolVarP(&placeOrdersFlag, "placeOrdersFlag", "o", false, "Specify if the order placement is allowed")

	rootCmd.AddCommand(startBotCmd)
	rootCmd.AddCommand(startHistoricalBotCmd)
...
var startBotCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		controller.StartAlphaHft(placeOrdersFlag)
	},
}

var startHistoricalBotCmd = &cobra.Command{
	Use:   "startHistoricalBot",
	Short: "Command to Start ALPHA-HFT Historical Bot",
	Long:  "Boots up the prices and run the strategies to generate alpha signals for historical data",
	Run: func(cmd *cobra.Command, args []string) {
		controller.StartAlphaHftHistoricalBot()
	},
}
...
var initialzeBotCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		controller.InitializeAlphaHftSOD(smartApiTotp,
			createTable,
			populateOldHistoricData,
			useBrokersSavedState,
			useProxy,
			instrumentFilterFlag,
			intervalFilterFlag)
	},
}
