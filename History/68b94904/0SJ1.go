	...
	for interval, intervalData := range a.historicalOHLCVTimeSeriesData {
		for token, tokenData := range intervalData {
			for exchange, ohlcvTimeSeries := range tokenData {
				...
				if len(ohlcvTimeSeries.Close) < 80 {
					logger.Log.Debug().
...
func createStrategies(ohlcvTimeSeries *common.OHLCVTimeSeries, sar []float64, rsi14 []float64, ichimoku *indicators.IchimokuCloud, adx []float64, plusDI []float64, minusDI []float64, sma20 []float64, ema10 []float64, ema20 []float64, ema60 []float64) []Strategy {
	strategies := []Strategy{
