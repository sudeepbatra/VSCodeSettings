...
class ApificationBreeze():
    ...
    def generate_headers(self, body):
        try:
            ...
            key_used_to_create_checksum = current_date+body+self.breeze.secret_key
            print("Key Used to Create Checksum: " + key_used_to_create_checksum)
            checksum = sha256(
    ...
    def make_request(self, method, endpoint, body, headers):
        try:
            ...
            if method == req_type.GET:
                res = requests.get(url=url, data=body, headers=headers, proxies={'http': proxy_url, 'https': proxy_url}, verify=False)
                return res
