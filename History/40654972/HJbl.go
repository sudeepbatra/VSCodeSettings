...
func (handler *CandlestickHandler) processTick(tick smartapi.TickParsedData) {
	...
	if exchangeTimestampInTick.Before(handler.marketOpenTime) || exchangeTimestampInTick.After(handler.marketCloseTime) {
		logger.Log.
			Info().
			Str("parser", "candlestick_from_tick").
	...
	for i := range handler.intervals {
		...
		if interval.IntervalStart.IsZero() {
			...
			Interface("interval", interval).
				Msg("Interval Start time set to: " + +" which was zero for interval: " + interval.Name)
		}
