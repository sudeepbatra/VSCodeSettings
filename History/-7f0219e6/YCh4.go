...
func unpackBestFiveData(data []byte) ([]BestFivePacket, []BestFivePacket, error) {
	...
	for i := 0; i < len(data); i += packetSize {
		...
		// Creating a BestFivePacket struct
		...
		// Appending to the appropriate array based on buy/sell flag
		if buySellFlag == 1 {
