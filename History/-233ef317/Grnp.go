...
func createSocketIOClient(uri string, authDict map[string]interface{}) (*socketio_client.Client, error) {
	...
	httpClient.Transport.(*http.Transport).TLSClientConfig.MinVersion = tls.VersionTLS13
	httpClient.Transport.(*http.Transport).TLSClientConfig.MaxVersion = tls.VersionTLS13
	httpClient.Transport.(*http.Transport).TLSClientConfig.CipherSuites = []uint16{tls.TLS_AES_128_GCM_SHA256}
	httpClient.Transport.(*http.Transport).TLSClientConfig.CurvePreferences = []tls.CurveID{tls.CurveP256, tls.X25519}
	...
	// Modify the Dialer field to use the custom HTTP client with headers
	opts.Dialer = func(network, addr string) (net.Conn, error) {
		return httpClient.Dial(network, addr)
	}
