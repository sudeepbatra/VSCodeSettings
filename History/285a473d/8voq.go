...
func BulkInsert(tableName string, data interface{}) error {
		...
		if item.Kind() == reflect.Struct {
			...
			for j := 0; j < item.NumField(); j++ {
				...
				if item.Type().Field(j).Type == reflect.TypeOf(time.Time{}) {
					...
					if timestampValue < 0 || timestampValue > 2147483647 {
						return fmt.Errorf("invalid Unix timestamp: %d", timestampValue)
					}
	...
	copyCount, err := AlphaHftDbConnPool.CopyFrom(context.Background(),
		...
		// Explicitly specify the format for the timestamp
		pgx.CopyFromWithDelimiter('\t', fmt.Sprintf("TIMESTAMPTZ, %s", strings.Join(fieldNames[1:], ", "))))
