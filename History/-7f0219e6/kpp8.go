...
func processResponse(respBytes []byte) *TickParsedData {
	...
	logger.Log.Trace().
		Str("msg", string(respBytes)).
		Msg("received response from websocket")
	...
	if parsedData.SubscriptionMode == SnapQuote {
		...
		best5Data, best5BuyData, best5SellData, err := unpackBestFiveData(respBytes[147:347])
		if err != nil {
			parsedData.Best5Data = best5Data
	...
	logger.Log.Info().
		Str("parsed tick data json", string(parsedDataJson)).
		Msg("Received market data")

	return &parsedData
}

func unpackBestFiveData(data []byte) ([]Best5Data, []Best5Data, []Best5Data, error) {
	var best5Data, best5BuyData, best5SellData []Best5Data
	...
	for i := 0; i < len(data); i += packetSize {
		...
		if buySellFlag == 1 {
			best5BuyData = append(best5BuyData, bestFivePacket)
		} else {
			best5SellData = append(best5SellData, bestFivePacket)
		}

		best5Data = append(best5Data, bestFivePacket)
	}

	logger.Log.Trace().
		Interface("best5Data", best5Data).
		Interface("best5BuyData", best5BuyData).
		Interface("best5SellData", best5SellData).
		Msg("tick data best five data")

	return best5Data, best5BuyData, best5SellData, nil
}
