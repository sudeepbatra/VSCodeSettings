...
func (sm *SocketManager) connect(isOrder, isOHLC bool) {
	...
	if isOrder && sm.socketOrder == nil {
		options := &gosocketio.ClientOptions{
			Transport: "websocket",
			Query:     authDict,
		}
		uriOrder := sm.config["LIVE_FEEDS_URL"]
		sm.socketOrder, _ = gosocketio.NewClient(strings.TrimSuffix(uriOrder, "/")+"/socket.io/", options)
	...
	if isOHLC && sm.socketOHLC == nil {
		...
		sm.socketOHLC, _ = gosocketio.NewClient(strings.TrimSuffix(uriOHLC, "/")+"/socket.io/", options)
	...
	if sm.socket == nil {
		...
		sm.socket, _ = gosocketio.NewClient(strings.TrimSuffix(uri, "/")+"/socket.io/", options)
