...
func init() {
	...
	initialzeBotCmd.Flags().BoolVarP(&useProxy, "useProxy", "u", false, "Specify true if you want to use proxy for monitoring the http requests")
...
var initialzeBotCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		controller.InitializeAlphaHftSOD(smartApiTotp, createTable, populateOldHistoricData, useBrokersSavedState, useProxy)
	},
}
