...
func FetchQuoteDataAndSaveInDB() {
	...
	cnbcQuotesURLReq, err := http.NewRequest("GET", cnbcQuotesURL, nil)
	if err != nil {
		logger.Log.Error().Err(err).Msg("error creating cnbc quotes url req")
		return
	}

	setRequestHeaders(cnbcQuotesURLReq)

	cnbcQuotesResp, err := client.Do(cnbcQuotesURLReq)
	if err != nil {
		logger.Log.Error().Err(err).Msg("error making cnbc quotes response")
		return
	}
	defer cnbcQuotesResp.Body.Close()

	cnbcQuotesRespBody, err := io.ReadAll(cnbcQuotesResp.Body)
	if err != nil {
		logger.Log.Error().Err(err).Msg("error reading cnbc quotes resp body")
		return
	}

	logger.Log.Trace().
		Interface("response", string(cnbcQuotesRespBody)).
		Msg("response from CNBC quote api")

	var quoteData QuoteData
	err = json.Unmarshal(cnbcQuotesRespBody, &quoteData)
	...
	var otherSymbolsData OtherSymbolsData
	err = json.Unmarshal(cnbcOtherSymbolsQuotesBody, &otherSymbolsData)
	...
	logger.Log.Trace().
		Interface("formattedQuoteData", otherSymbolsData).
		Msg("formattedQuoteData")

	for _, otherSymbolsData := range otherSymbolsData.OtherSymbolsQuotes.OtherSymbolsQuote {
		err := data.InsertRecord("other_symbols_quote", otherSymbolsData)
		if err != nil {
			logger.Log.Error().Err(err).Msg("Error storing otherSymbolsData to the DB:")
		}
	}
