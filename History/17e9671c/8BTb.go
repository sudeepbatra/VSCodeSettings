...
func (obm *OrderBookManager) ProcessAlphaSignals() {
	...
	for alphaSignal := range alphaSignalChannel {
		...
		if alphaSignal.Exchange != "NSE" || !present {
			logger.Log.Info().Str("symbol", alphaSignal.Symbol).
		...
		if !alphaSignal.IsStrategyLive {
			logger.Log.Info().
				Str("symbol", alphaSignal.Symbol).
				Str("exchange", alphaSignal.Exchange).
				Str("strategy", alphaSignal.StrategyName).
				Bool("isLive", alphaSignal.IsStrategyLive).
				Msg("Ignoring placing order for alpha signal as strategy is not live.")
