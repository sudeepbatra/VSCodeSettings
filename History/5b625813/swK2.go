...
func processResponse(respBytes []byte) {
	...
	parsedData["subscription_mode"] = unpackData(respBytes, 0, 1, "B")[0]
	parsedData["exchange_type"] = unpackData(respBytes, 1, 2, "B")[0]
	parsedData["token"] = parseTokenValue(respBytes[2:27])
	parsedData["sequence_number"] = unpackData(respBytes, 27, 35, "q")[0]
	parsedData["exchange_timestamp"] = unpackData(respBytes, 35, 43, "q")[0]
	parsedData["last_traded_price"] = unpackData(respBytes, 43, 51, "q")[0]
	...
	if subMode == quote || subMode == snapQuote {
		parsedData["last_traded_quantity"] = unpackData(respBytes, 51, 59, "q")[0]
		parsedData["average_traded_price"] = unpackData(respBytes, 59, 67, "q")[0]
		parsedData["volume_trade_for_the_day"] = unpackData(respBytes, 67, 75, "q")[0]
		parsedData["total_buy_quantity"] = unpackData(respBytes, 75, 83, "d")[0]
		parsedData["total_sell_quantity"] = unpackData(respBytes, 83, 91, "d")[0]
		parsedData["open_price_of_the_day"] = unpackData(respBytes, 91, 99, "q")[0]
		parsedData["high_price_of_the_day"] = unpackData(respBytes, 99, 107, "q")[0]
		parsedData["low_price_of_the_day"] = unpackData(respBytes, 107, 115, "q")[0]
		parsedData["closed_price"] = unpackData(respBytes, 115, 123, "q")[0]
	}

	if subMode == snapQuote {
		parsedData["last_traded_timestamp"] = unpackData(respBytes, 123, 131, "q")[0]
		parsedData["open_interest"] = unpackData(respBytes, 131, 139, "q")[0]
		parsedData["open_interest_change_percentage"] = unpackData(respBytes, 139, 147, "q")[0]
		parsedData["upper_circuit_limit"] = unpackData(respBytes, 347, 355, "q")[0]
		parsedData["lower_circuit_limit"] = unpackData(respBytes, 355, 363, "q")[0]
		parsedData["52_week_high_price"] = unpackData(respBytes, 363, 371, "q")[0]
		parsedData["52_week_low_price"] = unpackData(respBytes, 371, 379, "q")[0]
		best5BuyAndSellData := parseBest5BuyAndSellData(respBytes[147:347])
