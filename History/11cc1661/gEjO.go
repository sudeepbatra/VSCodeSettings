...
func (ccdlhr ChikouCrossDownLowHighRule) IsSatisfied(lastIndex int) bool {
	...
	satisfied := chikouValueAtTimeDelay < lowAtTimeDelay &&
		chikouValueAtTimeDelayMinus1 > highAtTimeDelayMinus1

	if satisfied && ccdlhr.traceRule {
		logger.Log.Info().
			Int("lastIndex", lastIndex).
			Float64("lowAtTimeDelay", lowAtTimeDelay).
			Float64("chikouValueAtTimeDelay", chikouValueAtTimeDelay).
			Float64("highAtTimeDelayMinus1", highAtTimeDelayMinus1).
			Float64("chikouValueAtTimeDelayMinus1", chikouValueAtTimeDelayMinus1).
			Msg("ChikouCrossDownLowHighRule satisfied")
	}
