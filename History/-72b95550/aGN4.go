...
func (a *ApificationBreeze) generateHeadersOld(body string) http.Header {
	...
	checksum := fmt.Sprintf("%x", sha256.Sum256([]byte(formattedTime+body+a.secretKey)))
	headers := http.Header{}
	headers.Set("Content-Type", "application/json")
	headers.Set("X-Checksum", "token "+checksum)
