...
func unpackOrdersData(data []byte) []Order {
	...
	for i := 0; i < len(data)/20; i++ {
		start := i * 20
		price, _ := unpackData(data[start : start+8])
		quantity, _ := unpackData(data[start+8 : start+12])
		orders, _ := unpackData(data[start+12 : start+16])

		// Convert quantity from int64 to uint32, considering non-negative values
		quantityUint32 := uint32(quantity)
		if quantity < 0 {
			// Handle negative values accordingly, as uint32 cannot represent negative values
			// For example, set quantityUint32 to 0 or another appropriate value.
			quantityUint32 = 0
		}
		...
		ordersData = append(ordersData, Order{priceFloat, quantityUint32, orders})
	}
