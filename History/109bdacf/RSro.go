...
var (
	feedToken           = "eyJhbGciOiJIUzUxMiJ9.eyJ1c2VybmFtZSI6IlMxNjMyNTg1IiwiaWF0IjoxNjkyNzU4ODUyLCJleHAiOjE2OTI4NDUyNTJ9.1jnIYoOWnWzyn0IWBbbiENPkdAPso9Law0j7tv1OM9mVczvmEttHiWb5dniKIfkL9sgZ6SJvMsz0WXCtrJR46A"
	renewToken          = "eyJhbGciOiJIUzUxMiJ9.eyJ0b2tlbiI6IlJFRlJFU0gtVE9LRU4iLCJpYXQiOjE2OTI3NTg4NTV9.XJyf8H9Nc4hKgvCdu2pde5L8vuxbWYrCCktt0FsMVGY-QmV2M0MJqVTMJaAq9djLBdBbo_2HPE14CZYP1MeBPw"
	subscriptionModeMap = map[int]string{
		1: "LTP",
		2: "QUOTE",
		3: "SNAP_QUOTE",
	}
...
func SmartAPIMarketDataWS() {
	...
	// Customize subscription parameters
	correlationID := "abcde12345"
	action := SubscribeAction
	mode := SnapQuote

	// Define tokenLists separately
	nseTokenList := []RequestTokenList{
		{
			ExchangeType: NseCm,
	...
	singleSubscription := Subscription{
		...
		Params: RequestParams{
			Mode:       mode,
			TokenLists: nseTokenList,
		},
	}

	// Subscribe using the generalized function with a single subscription
	subscribe(conn, singleSubscription)
}

func subscribeOld(conn *websocket.Conn) {
...
func subscribe(conn *websocket.Conn, subscriptions ...Subscription) {
	for _, sub := range subscriptions {
		reqBytes, err := json.Marshal(sub)
		if err != nil {
			log.Println("Error marshaling request:", err)
			return
		}
...
func processResponse(respBytes []byte) {
	log.Info("Received response:", respBytes)

	var parsedData ParsedData

	if bytes.Equal(respBytes, []byte("pong")) {
		log.Info("respBytes is 'pong'")
		return
	}
	...
	log.Info("1 ParsedData: ", parsedData)
	...
	log.Info("2 ParsedData: ", parsedData)
}
