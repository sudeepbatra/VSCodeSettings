...
func FetchQuoteDataAndSaveInDB() {
	...
	logger.Log.Info().Interface("quoteData", quoteData).Msg("quoteData")

	for _, fairValueQuote := range quoteData.FairValueQuoteResult.FairValueQuote {
		logger.Log.Info().Interface("fairValueQuote", fairValueQuote).Msg("fairValueQuote")

		fmt.Println("Storing FairValueQuote to the DB...")

		err := data.InsertRecord("fair_value_quotes", fairValueQuote)
		if err != nil {
			fmt.Println("Error storing FairValueQuote to the DB:", err)
		} else {
			fmt.Println("Stored FairValueQuote to the DB!")
		}
	}
}
