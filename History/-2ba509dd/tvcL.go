...
func Login(sessionToken string, useProxy bool) {
	...
	orderRequestBody, err := createOrderRequestBody("ZOMLIM", "NSE", "cash", "buy", "limit", 0.0, 1, 80, "day", "", 0, "", "", 0, "Limit order from Breeze API for Zomato")
	if err != nil {
		log.Error("Error while trying to create order request body:", err)
	...
	if err != nil {
		log.Error("Error while trying to execute placeOrder:", err)

		return
	}

	if breezeOrder.Order.Message != nil {
		log.Infof("breezeOrder Message: %s", *breezeOrder.Order.Message)
		orderID, err := breezeOrder.getOrderID()
		if err != nil {
			log.Error("Error while trying get the breeze order id:", err)
		}

		log.Info("breezeOrder OrderID:", orderID)

		orderDetailRequestBody := createOrderDetailRequestBody("NSE", orderID)
		orderDetail, err := apiHandler.getOrderDetail(useProxy, orderDetailRequestBody)
		if err != nil {
			log.Error("Error while trying to execute orderDetail:", err)
		}

		log.Infof("orderDetail: %s", orderDetail)
	}
