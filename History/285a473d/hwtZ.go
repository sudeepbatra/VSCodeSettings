...
// 		firstItemType := reflect.TypeOf(firstItem.Interface())
// 		fmt.Println("Type of first item:", firstItemType)

// 		fmt.Printf("Value of first item: %v\n", firstItem.Interface())
...
// 				tag := field.Tag.Get("json") // Use the appropriate tag (e.g., "json", "xml")
...
func getStructFieldNames(data interface{}) []string {
	...
	if v.Kind() == reflect.Slice && v.Len() > 0 {
		...
		// Dereference the pointer if firstItem is a pointer
		if firstItem.Kind() == reflect.Ptr {
			firstItem = firstItem.Elem()
		}
