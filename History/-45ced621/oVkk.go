...
func GetInstrumentsSymbolAndTokenMapping() (map[smartapi.InstrumentKey]int, map[int]smartapi.InstrumentKey, error) {
	...
	logger.Log.Info().Msg("instruments: " + fmt.Sprintf("%+v", instruments))

	symbolToTokenMap := make(map[smartapi.InstrumentKey]int)
	tokenToSymbolMap := make(map[int]smartapi.InstrumentKey)

	for _, instrument := range instruments {
		token, err := strconv.Atoi(instrument.Token)
		if err != nil {
			logger.Log.Error().Err(err).Msgf("Error converting token %s to int", instrument.Token)
			continue
		}

		// Create an InstrumentKey for the symbol and exchange combination
		key := smartapi.InstrumentKey{Symbol: instrument.Symbol, Exchange: instrument.Exchange}

		// Create mappings
		symbolToTokenMap[key] = token
		tokenToSymbolMap[token] = key
	}
