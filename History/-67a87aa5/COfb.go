	...
	for _, instrument := range instruments {
		if instrument.ExchSeg == "NFO" &&
			...
			if instrument.Name == "NIFTY" &&
				...
				instrumentStrike <= nifty50MaxCEStrike &&
				instrumentStrike >= nifty50MinCEStrike &&
			...
			if instrument.Name == "NIFTY" &&
				...
				instrumentStrike <= nifty50MaxPEStrike &&
				instrumentStrike >= nifty50MinPEStrike &&
				differenceInDays < maxOptionsExpiryDateFromToday {
				nfoTokens = append(nfoTokens, instrument.Token)
			}

			if instrument.Name == "BANKNIFTY" &&
				instrument.OptionType == "CE" &&
				instrumentStrike <= niftybankMaxCEStrike &&
				instrumentStrike >= niftybankMinCEStrike &&
				differenceInDays < maxOptionsExpiryDateFromToday {
				nfoTokens = append(nfoTokens, instrument.Token)
			}
			...
			if instrument.Name == "BANKNIFTY" &&
				instrument.OptionType == "PE" &&
				instrumentStrike <= niftybankMaxPEStrike &&
				instrumentStrike >= niftybankMinPEStrike &&
				differenceInDays < maxOptionsExpiryDateFromToday {
				nfoTokens = append(nfoTokens, instrument.Token)
			}
		}
