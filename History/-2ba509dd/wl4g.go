...
func Login(sessionToken string, useProxy bool) {
	...
	orderParams := createOrderRequestBody("AXIBAN", "NSE", "futures", "buy", "limit", 0.0, 10, 1500.0, "day", "", 0, "", "", 0, "Sample remark")
	brokerageCharges, err := apiHandler.placeOrder(useProxy, orderParams)
	if err != nil {
		log.Error("Error while trying to execute brokerageCharges:", err)
