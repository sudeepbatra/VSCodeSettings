...
func subscribe(conn *websocket.Conn, correlationID string, action Action, mode Mode, tokenLists ...[]RequestTokenList) {
	request := Subscription{
		CorrelationID: correlationID,
		Action:        action,
		Params: RequestParams{
			Mode:       mode,
			TokenLists: []RequestTokenList{}, // Initialize an empty slice
		},
	}

	// Concatenate the provided tokenLists
	for _, tl := range tokenLists {
		request.Params.TokenLists = append(request.Params.TokenLists, tl...)
	}
