...
const (
	...
	tradeBookURL          = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/order/v1/getTradeBook"
	getLTPDataURL         = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/order/v1/getLtpData"
	getHoldingsURL        = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/portfolio/v1/getHolding"
	getPositionsURL       = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/order/v1/getPosition"
	GetFundsAndMarginsURL = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/user/v1/getRMS"
)
...
func GetInstrumentsScripMasterData(client *http.Client) ([]InstrumentRecord, error) {
	...
	for i := range instrumentRecords {
		...
		if instrumentRecords[i].ExchSeg == "NFO" &&
			(instrumentRecords[i].InstrumentType == "OPTIDX" || instrumentRecords[i].InstrumentType == "OPTSTK") {
			if isCallOption(instrumentRecords[i].Symbol) {
				instrumentRecords[i].OptionType = "CE"
			} else if isPutOption(instrumentRecords[i].Symbol) {
				instrumentRecords[i].OptionType = "PE"
			} else {
				instrumentRecords[i].OptionType = ""
			}
		}
	}
...
func isCallOption(optionString string) bool {
	parts := strings.Split(optionString, "CE")
	return len(parts) == 2
}

func isPutOption(optionString string) bool {
	parts := strings.Split(optionString, "PE")
	return len(parts) == 2
}
...
func (s *SmartApiApplication) GetTradeBook(client *http.Client) (*TradeBookResponse, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}
	...
	resBody, err := s.requestSmartAPI(client, httpMethod, tradeBookURL, nil, nil, nil)
	if err != nil {
		return nil, err
	}

	logger.Log.Debug().Str("data", string(resBody)).Msg("get trade book result")

	var tradeBookResponse TradeBookResponse

	err = json.Unmarshal(resBody, &tradeBookResponse)
	if err != nil {
		return nil, err
	}
...
func (s *SmartApiApplication) GetLtpData(client *http.Client, exchange, tradingSymbol,
	symbolToken string,
) (*LTPDataResponse, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}
	...
	resBody, err := s.requestSmartAPI(client, method, getLTPDataURL, nil, payload, nil)
	if err != nil {
		return nil, err
	}

	logger.Log.Debug().Str("data", string(resBody)).Msg("get LTP data result")

	var ltpDataResponse LTPDataResponse

	err = json.Unmarshal(resBody, &ltpDataResponse)
	if err != nil {
		return nil, err
	}

	return &ltpDataResponse, nil
}

func (s *SmartApiApplication) GetHoldings(client *http.Client) (*HoldingsResponse, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}

	resBody, err := s.requestSmartAPI(client, "GET", getHoldingsURL, nil, nil, nil)
	if err != nil {
		return nil, err
	}

	logger.Log.Debug().Str("data", string(resBody)).Msg("get holdings result")

	var holdingsResponse HoldingsResponse

	err = json.Unmarshal(resBody, &holdingsResponse)
	if err != nil {
		return nil, err
	}
...
func (s *SmartApiApplication) GetPositions(client *http.Client) (*PositionsResponse, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}

	resBody, err := s.requestSmartAPI(client, "GET", getPositionsURL, nil, nil, nil)
	if err != nil {
		return nil, err
	}

	logger.Log.Debug().Str("data", string(resBody)).Msg("get positions result")

	var positionsResponse PositionsResponse

	err = json.Unmarshal(resBody, &positionsResponse)
	if err != nil {
		return nil, err
	}
...
func (s *SmartApiApplication) GetFundsAndMargins(client *http.Client) (*FundsAndMargin, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}

	resBody, err := s.requestSmartAPI(client, "GET", GetFundsAndMarginsURL, nil, nil, nil)
	if err != nil {
		return nil, err
	}

	logger.Log.Debug().Str("data", string(resBody)).Msg("get funds and margin result")

	var fundsAndMarginResponse FundsAndMargin

	err = json.Unmarshal(resBody, &fundsAndMarginResponse)
	if err != nil {
		return nil, err
	}

	return &fundsAndMarginResponse, nil
}
