...
const (
	...
	niftyMaxCEMultiplier          = 5.0
	niftyMinCEMultiplier          = 1.0
	niftyMaxPEMultiplier          = 1.0
	niftyMinPEMultiplier          = 5.0
	niftybankMaxCEMultiplier      = 10.0
	niftybankMinCEMultiplier      = 2.0
	niftybankMaxPEMultiplier      = 2.0
	niftybankMinPEMultiplier      = 10.0
)
...
func calculateStrikePriceRanges(lastTradePrice float64, ceMultiplier, peMultiplier float64) strikePriceRanges {
	return strikePriceRanges{
		MaxCE: lastTradePrice + ceMultiplier*100.0,
		MinCE: lastTradePrice - ceMultiplier*100.0,
		MaxPE: lastTradePrice + peMultiplier*100.0,
		MinPE: lastTradePrice - peMultiplier*100.0,
	}
}
...
func getNFOTokens() ([]string, error) {
	...
	nifty50LastTradePrice, niftybankLastTradePrice := getNifty50AndBankNiftyLTP(client)

	niftyStrikeRanges := calculateStrikePriceRanges(nifty50LastTradePrice, niftyMaxCEMultiplier, niftyMinCEMultiplier)
	niftybankStrikeRanges := calculateStrikePriceRanges(niftybankLastTradePrice, niftybankMaxCEMultiplier, niftybankMinCEMultiplier)

	for _, instrument := range instruments {
		if instrument.ExchSeg == "NFO" &&
			instrument.InstrumentType == "OPTIDX" &&
			(instrument.Name == "NIFTY" || instrument.Name == "BANKNIFTY") {

			instrumentStrike, err := strconv.ParseFloat(instrument.Strike, 64)
			if err != nil {
				logger.Log.Err(err).Interface("instrument", instrument).Msg("Error parsing instrument strike price")
				continue
			}

			instrumentStrike /= 100.0

			parsedExpiryDate, err := time.Parse(dateFormatLayout, instrument.Expiry)
			if err != nil {
				logger.Log.Err(err).Interface("instrument", instrument).Msg("Error parsing instrument expiry date")
				continue
			}

			differenceInDays := int(parsedExpiryDate.Sub(now).Hours() / 24)

			isValidOption := differenceInDays < maxOptionsExpiryDateFromToday

			switch {
			case instrument.Name == "NIFTY" && instrument.OptionType == "CE" && instrumentStrike <= niftyStrikeRanges.MaxCE && instrumentStrike >= niftyStrikeRanges.MinCE && isValidOption:
				nfoTokens = append(nfoTokens, instrument.Token)
			case instrument.Name == "NIFTY" && instrument.OptionType == "PE" && instrumentStrike <= niftyStrikeRanges.MaxPE && instrumentStrike >= niftyStrikeRanges.MinPE && isValidOption:
				nfoTokens = append(nfoTokens, instrument.Token)
			case instrument.Name == "BANKNIFTY" && instrument.OptionType == "CE" && instrumentStrike <= niftybankStrikeRanges.MaxCE && instrumentStrike >= niftybankStrikeRanges.MinCE && isValidOption:
				nfoTokens = append(nfoTokens, instrument.Token)
			case instrument.Name == "BANKNIFTY" && instrument.OptionType == "PE" && instrumentStrike <= niftybankStrikeRanges.MaxPE && instrumentStrike >= niftybankStrikeRanges.MinPE && isValidOption:
				nfoTokens = append(nfoTokens, instrument.Token)
			}
		}
	}
...
func getNifty50AndBankNiftyLTP(client *http.Client) (float64, float64) {
	...
	nifty50IndiceLastTradePrice := nifty50LTPResponse.Data.Ltp
	logger.Log.Debug().
		Float64("niftyIndiceLastTradePrice", nifty50IndiceLastTradePrice).
		Msg("Nifty 50 Indice Last Value: ")
	...
	niftybankIndiceLastTradePrice := niftybankLTPResponse.Data.Ltp
	logger.Log.Debug().
		Float64("niftybankIndiceLastTradePrice", niftybankIndiceLastTradePrice).
		Msg("Nifty Indice Last Value: ")
	return nifty50IndiceLastTradePrice, niftybankIndiceLastTradePrice
}
