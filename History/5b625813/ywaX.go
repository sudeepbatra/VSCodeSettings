...
func processResponse(respBytes []byte) {
	...
	subscriptionMode := int(respBytes[0])
	exchangeType := int(respBytes[1])
	fmt.Println(exchangeType)
	token := string(respBytes[2:27])
	sequenceNumber := binary.LittleEndian.Uint64(respBytes[27:35])
	fmt.Println(sequenceNumber)
	exchangeTimestamp := binary.LittleEndian.Uint64(respBytes[35:43])
	fmt.Println(exchangeTimestamp)
	// ... continue parsing other fields

	// Handle different response modes
	switch subscriptionMode {
	case 1: // LTP
		ltp := byteArrayToLittleEndianInt32(respBytes[43:51])
		fmt.Printf("Received LTP for token %s: %d\n", token, ltp)
	case 2: // Quote
		// Parse quote data and fields
		// ...
	case 3: // Snap Quote
		// Parse snap quote data and fields
		// ...
	default:
		log.Printf("Unknown subscription mode: %d\n", subscriptionMode)
	}
}
