...
func CalculateIchimokuCloud(high, low, close []float64, tenkanPeriod, kijunPeriod, senkouBPeriod int) *IchimokuCloud {
	...
	ic.SenkouA = make([]float64, length+26+1)
	ic.SenkouB = make([]float64, length+26+1)
	...
	for i := senkouBPeriod; i < length; i++ {
		...
		ic.Kijun[i] = kijunSen
		ic.SenkouA[i+26+1] = senkouSpanA
		ic.SenkouB[i+26+1] = senkouSpanB
		ic.Chikou[i-26+1] = chikouSpan
	}
