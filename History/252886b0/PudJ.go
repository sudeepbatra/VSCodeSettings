...
func generateSession(appName, apiKey, apiSecret, sessionToken string) (*ApificationBreeze, error) {
	...
	try {
		...
		if responseData["Success"] != nil {
			...
			if len(sessionParts) >= 2 {
				...
				b.SessionKey = sessionParts[1]
			} else {
				return fmt.Errorf(except_message.AUTHENTICATION_EXCEPTION.value)
			}
		} else {
			return fmt.Errorf(except_message.AUTHENTICATION_EXCEPTION.value)
		}
...
func (a *ApificationBreeze) getCustomerDetails(apiSession string) (map[string]interface{}, error) {
	if apiSession == "" || apiSession == nil {
		return a.validationErrorResponse(respMessage.API_SESSION_ERROR.value), nil
	}
