...
func NewAlphaSignal(token string, exchangeCode int, exchange, interval string, lastBarStartDuration time.Time,
	strategy, signal string, signalGenerationTime time.Time, price, o, h, l, c, v float64, alphaSignalReason string,
) (*AlphaSignal, error) {

	switch strategy {
	case "Long", "Short", "Long_Exit", "Short_Exit":
		// Valid strategy, continue with initialization
	default:
		// Invalid strategy, return an error
