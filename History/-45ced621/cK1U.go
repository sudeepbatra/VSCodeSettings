...
func GetHistoricalDataForInterval(interval string) ([]smartapi.CandleData, error) {
	query := fmt.Sprintf(HistoricalCandleDataIntervalGetQuery, strings.ToLower(interval))
...
func GetOHLCVTimeSeriesIntervalToken(interval string, token int, exchange string) (common.OHLCVTimeSeries, error) {
	...
	query := fmt.Sprintf(OHLCVTimeSeriesQuery, strings.ToLower(interval), token)
	...
	ohlcvTimeSeriesForToken := common.NewOHLCVTimeSeries(token, rowCount)
