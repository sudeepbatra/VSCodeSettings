...
func NewAlphaSignalProcessor() *AlphaSignalProcessor {
	logger.Log.Info().Str("handler", "alpha_signal_processor").Msg("New AlphaSignalProcessor Created!")
	return &AlphaSignalProcessor{}
}

func (p *AlphaSignalProcessor) ProcessAlphaSignals(useProxy bool, placeOrdersFlag bool) {
	alphaSignalChannel := AlphaSignalManager.Subscribe()
	client := common.GetClient(useProxy)

	logger.Log.Info().Str("handler", "alpha_signal_processor").Msg("AlphaSignalProcessor started!")
	...
	for alphaSignal := range alphaSignalChannel {
		...
		if alphaSignal.Signal == "LONG" {
			...
			if placeOrdersFlag {
				...
				logger.Log.Info().Interface("alphaSignal", alphaSignal).Msg("Order not placed for alpha signal as flag not set")
			}
		}
