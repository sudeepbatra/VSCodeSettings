...
type OrderDetailResponse struct {
	OrderDetail []OrderDetail `json:"Success"`
	Status      int           `json:"Status"`
	Error       *string       `json:"Error"`
}
...
type OrderDetail struct {
	...
	ExchangeAckDate           string `json:"exchange_acknowledgement_date"`
	SLTPPrice                 string `json:"SLTP_price"`
	ExchangeAcknowledgeNumber string `json:"exchange_acknowledge_number"`
...
func (a *ApificationBreeze) getOrderDetail(useProxy bool, requestBody interface{}) (orderDetailResponse OrderDetailResponse, err error) {
	errorHandler := func(err error) {
		log.Debugf("getOrderDetail Error: %s", err)
	}

	err = a.getAPIResponse(http.MethodGet, useProxy, orderEndpoint, requestBody, &orderDetailResponse, errorHandler)
...
func createOrderDetailRequestBody(exchangeCode, order_id string) map[string]string {
	quotesRequestBody := make(map[string]string)
	quotesRequestBody["exchange_code"] = exchangeCode
	quotesRequestBody["order_id"] = order_id
...
func createOrderListRequestBody(exchange_code, fromDate, toDate, productType, action, stockCode string) map[string]string {
	quotesRequestBody := make(map[string]string)
	quotesRequestBody["exchange_code"] = exchangeCode
