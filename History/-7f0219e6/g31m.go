...
func processResponse(respBytes []byte) *TickParsedData {
	...
	if parsedData.SubscriptionMode == SnapQuote {
		...
		parsedData.Best20BuyData = unpackOrdersData(respBytes[147:347])
...
func unpackOrdersData(data []byte) []Order {
	var ordersData []Order

	for i := 0; i < len(data)/20; i++ {
		start := i * 20
		quantity = int32(binary.LittleEndian.Uint32(data[start : start+4]))
		order.Price = int32(binary.LittleEndian.Uint32(data[start+4 : start+8]))
		order.NumberOfOrders = int16(binary.LittleEndian.Uint32(data[start+8 : start+10]))

		// Convert quantity and orders from int64 to uint32, considering non-negative values
		quantityUint32 := uint32(quantity)
		if quantity < 0 {
			// Handle negative values accordingly
			quantityUint32 = 0
		}

		ordersUint32 := uint32(orders)
		if orders < 0 {
			// Handle negative values accordingly
			ordersUint32 = 0
		}

		// Convert price from int64 to float64
		priceFloat := float64(price)

		ordersData = append(ordersData, Order{priceFloat, quantityUint32, ordersUint32})
	}
