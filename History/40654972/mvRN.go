...
func (handler *CandlestickHandler) processTick(tick smartapi.TickParsedData) {
	...
	if !config.Config.RunTimeConfig.SimulationMode && (exchangeTimestampInTick.Before(handler.marketOpenTime) || exchangeTimestampInTick.After(handler.marketCloseTime)) {
		logger.Log.
			Info().
			Str("parser", "candlestick_from_tick").
