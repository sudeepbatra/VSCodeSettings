...
func (handler *CandlestickHandler) processTick(data smartapi.TickParsedData) {
	exchangeTimestamp := data.ExchangeTimestamp

	logger.Log.Trace().
		Str("parser", "candlestick_from_tick").
		Interface("tick", data).
		Str("timestamp", exchangeTimestamp.Format("2006-01-02 15:04:05")).
		Msg("Tick received: ")

	handler.mu.Lock()
	defer handler.mu.Unlock()

	if exchangeTimestamp.Before(handler.marketOpenTime) || exchangeTimestamp.After(handler.marketCloseTime) {
		logger.Log.
			Info().
			Str("parser", "candlestick_from_tick").
			Msg("Tick outside the market hours: " + exchangeTimestamp.Format("2006-01-02 15:04:05"))
	...
	for i := range handler.intervals {
		...
		if interval.IntervalStart.IsZero() {
			interval.IntervalStart = handler.getIntervalStartTime(exchangeTimestamp, interval.Duration)
			logger.Log.Info().
				Str("parser", "candlestick_from_tick").
		...
		if exchangeTimestamp.Before(interval.IntervalStart.Add(interval.Duration)) {
			logger.Log.Debug().
				Str("parser", "candlestick_from_tick").
