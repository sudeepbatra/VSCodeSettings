...
func (o *OrderResponse) getOrderID() (string, error) {
	if o.Status == 200 && o.Order.Message != nil {
		pattern := `reference no (\d{12}[A-Z]\d{9})`
		...
		if len(match) >= 2 {
			...
			log.Println("Order ID not found in the text.")
			return "", errors.New("Could not get the order id from the message")
		}
	}

	return "", errors.New("Invalid status or message is nil")
}
...
func createOrderRequestBody(stockCode, exchangeCode, productType, action, orderType string, stopLoss float64, quantity int, price float64, validity string, validityDate string, disclosedQuantity int, expiryDate string, right string, strikePrice int, userRemark string) interface{} {
