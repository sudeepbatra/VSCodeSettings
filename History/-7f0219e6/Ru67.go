...
func unpackOrdersData(binaryData []byte) {
	for i := 0; i < 5; i++ {
		start := 200 + i*20
		price, _ := unpackData(respBytes[start : start+8])
		quantity, _ := unpackData(respBytes[start+8 : start+12])
		orders, _ := unpackData(respBytes[start+12 : start+16])

		bestFiveBuyData[i] = struct {
			Price    float64
			Quantity uint32
			Orders   uint32
