...
func InitializeTables() {
	...
	_, err = AlphaHftDbConn.Exec(context.Background(), CreateCnbcFairValueQuoteTableQuery)
	if err != nil {
		logger.Log.Error().Err(err).Msg("Error in creating cnbc fair value quote table")
	}

	_, err = AlphaHftDbConn.Exec(context.Background(), CreateCnbcOtherSymbolsQuoteTableQuery)
	if err != nil {
		logger.Log.Error().Err(err).Msg("Error in creating cnbc other symbols quote table")
	}
