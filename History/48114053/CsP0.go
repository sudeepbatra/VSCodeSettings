...
func FetchQuoteDataAndSaveInDB() {
	...
	cnbcQuotesURLResp, err := client.Do(cnbcQuotesURLReq)
	if err != nil {
		logger.Log.Error().Err(err).Msg("Error making cnbc quotes url resp")
		return
	}
	defer cnbcQuotesURLResp.Body.Close()

	cnbcQuotesURLBody, err := io.ReadAll(cnbcQuotesURLResp.Body)
	if err != nil {
	...
	logger.Log.Trace().
		Interface("response", string(cnbcQuotesURLBody)).
		Msg("response from CNBC quote api")

	var quoteData QuoteData
	err = json.Unmarshal(cnbcQuotesURLBody, &quoteData)
