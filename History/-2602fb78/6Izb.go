...
func (cculhr ChikouCrossUpLowHighRule) IsSatisfied(lastIndex int) bool {
	...
	if satisfied && cculhr.traceRule {
		logger.Log.Info().
			Int("lastIndex", lastIndex).
			Float64("highAtTimeDelay", highAtTimeDelay).
			Float64("chikouAtTimeDelay", chikouAtTimeDelay).
			Float64("lowAtTimeDelayMinus1", lowAtTimeDelayMinus1).
			Float64("chikouAtTimeDelayMinus1", chikouAtTimeDelayMinus1).
			Msg("ChikouCrossUpLowHighRule satisfied")
	}
