...
func (t *TickDataStorage) StoreTickData() {
	...
	for {
		...
		case tickData, ok := <-parsedTickData:
			...
			modifiedTickData := smartapi.TickParsedDataStorage{}
			data.CopyStructExcludingFields(tickData, &modifiedTickData, excludedFields)

			logger.Log.Trace().Interface("tickData", modifiedTickData).Msg("Storing Tick Data to the buffer...")
