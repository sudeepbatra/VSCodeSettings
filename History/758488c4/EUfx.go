	...
	for {
		...
		if err != nil {
			...
			for {
				...
				if err == nil {
					logger.Log.Warn().
						Msg("Reconnected to smart api websocket successfully. Some messages may have been lost.")

					logger.WebSocketLog.Warn().
						Msg("Reconnected to smart api websocket successfully. Some messages may have been lost.")
				...
				logger.Log.Error().
					Err(err).
					Msg("Failed to reconnect. Retrying in 5 seconds...")

				logger.WebSocketLog.Error().
					Err(err).
					Msg("Failed to reconnect. Retrying in 5 seconds...")
...
func (s *SmartApiApplication) sendHeartbeats() {
	for {
		logger.Log.Debug().
			Msg("Sending heartbeats to the smart api websocket...")

		logger.WebSocketLog.Debug().
			Msg("Sending heartbeats to the smart api websocket...")

		time.Sleep(heartBeatInterval)

		logger.Log.Debug().
			Msg("Sent heartbeats to the smart api websocket!")
		...
		if err := s.WsConn.WriteMessage(websocket.TextMessage, []byte(heartBeatMessage)); err != nil {
			logger.Log.Error().
				Err(err).
				Msg("error in sending heartbeats to the smart api websocket")
...
func (s *SmartApiApplication) Subscribe(subRequest Subscription) error {
	...
	if err != nil {
		...
		logger.WebSocketLog.Error().Str("request", string(reqBytes)).Msg("Error in decoding subscription request")

		return err
	}

	logger.Log.Info().Msg("Sending Subscribe to the smart api websocket")
	logger.WebSocketLog.Info().Msg("Sending Subscribe to the smart api websocket")

	err = s.WsConn.WriteMessage(websocket.TextMessage, reqBytes)
	if err != nil {
		logger.Log.Error().
			Str("request", string(reqBytes)).
			Msg("Error in writing subscription request to smart api websocket")
...
func (s *SmartApiApplication) Unsubscribe(subRequest Subscription) error {
	...
	if err != nil {
		logger.Log.Error().
			Str("request", string(reqBytes)).
			Msg("Error in decoding subscription request")
	...
	logger.WebSocketLog.Info().Msg("Sending Unsubscribe to the smart api websocket")
