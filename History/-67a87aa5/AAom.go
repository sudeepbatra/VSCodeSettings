...
func getNFOTokens() ([]string, error) {
	...
	nifty50LTP, niftybankLTP := getNifty50AndBankNiftyLTP(client)

	niftyStrikeRanges := calculateStrikePriceRanges(nifty50LTP, niftyMaxMultiplier, niftyMinMultiplier)

	niftybankStrikeRanges := calculateStrikePriceRanges(niftybankLTP, niftybankMaxMultiplier, niftybankMinMultiplier)
	...
	for _, instrument := range instruments {
		if instrument.ExchSeg == "NFO" &&
			...
			isExpiryClose := differenceInDays < maxOptionsExpiryDateFromToday
