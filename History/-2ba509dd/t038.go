...
func Login(sessionToken string, useProxy bool) {
	...
	tradeDetailRequestBody := createBrokerageChargesRequestBody("ZOMLIM", "NSE", "", "limit", "80", "buy", "100")
	tradeDetail, err := apiHandler.getBrokerageCharges(useProxy, tradeDetailRequestBody)
	if err != nil {
		log.Error("Error while trying to execute getTradeDetail:", err)
