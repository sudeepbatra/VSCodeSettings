...
const (
	...
	tradeBookURL        = "https://apiconnect.angelbroking.com/rest/secure/angelbroking/order/v1/getOrderBook"
)
...
func (s *SmartApiApplication) GetTradeBook(client *http.Client) (*GetTradeBookResponse, error) {
	err := s.CheckAccessTokenNotEmpty()
	if err != nil {
		return nil, err
	}
	...
	resBody, err := s.requestSmartAPI(client, httpMethod, url, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	...
	var getTradeBookResponse GetTradeBookResponse

	err = json.Unmarshal(resBody, &getTradeBookResponse)
	if err != nil {
		return nil, err
	}

	return &getTradeBookResponse, nil
}
