...
func GetInstrumentsSymbolAndTokenMapping() (map[string]int, map[int]string, error) {
	...
	symbolToToken := make(map[string]int)
	tokenToSymbol := make(map[int]string)

	for _, instrument := range instruments {
		// Create mappings
		symbolToToken[instrument.Symbol] = instrument.Token
		tokenToSymbol[instrument.Token] = instrument.Symbol
	}
